...
class GLFM(nn.Module):
    ...
    def forward(self, x):
        xl = self.local_att(x)
        xg = self.global_att(x)
        xlg = torch.cat((xl, xg), dim=2)
        
        weights = F.softmax(xlg, dim=2)
...
# class GLFM(nn.Module):
#     def __init__(self, channels=64, r=4):
#         sdwoner(GLFM, self).__init__()
#         # Existing initialization code remains unchanged...

#     def forward(self, x):
#         xl = self.local_att(x)
#         xg = self.global_att(x)
#         # Stack xl and xg along a new dimension (dim=1) and apply softmax
#         xlg = torch.cat((xl, xg), dim=1)
#         xlg = xlg.view(1, 2, -1)  # Reshape for softmax
#         weights = F.softmax(xlg, dim=1)  # Apply softmax
#         weights = weights.view(2, 1, x.size(1), 1, 1)  # Reshape back to match the channel dimension
...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        weight1, weight2 = self.gl_fm(x)
        result1 = x * weight1.expand_as(x)
        result2 = x * weight2.expand_as(x)
        final_result = result1 + result2
        
        x = self.bn_2(final_result)
        x = self.relu(x)
        x = self.conv1x1_output(x)
        x = self.bn_output(x)
