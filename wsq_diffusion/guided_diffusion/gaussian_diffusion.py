...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        #采样
        model_output, attn_map = model(x, self._scale_timesteps(t), **model_kwargs)
#         selected_attn_map = attn_map[0, 0].cpu().detach()

# # 将选中的注意力图正规化到[0, 1]范围内进行可视化
#         # 假设 selected_attn_map 是你从模型获取的注意力图，并已经被移动到CPU并且脱离了计算图

# # 正规化注意力图
#         normalized_attn_map = (selected_attn_map - torch.min(selected_attn_map)) / (torch.max(selected_attn_map) - torch.min(selected_attn_map))

#         # 确保normalized_attn_map是2D的，这对于灰度图像是必要的
#         # 如果它已经是2D的（例如，形状为 [height, width]），unsqueeze操作将不会改变其原本的2D结构
#         normalized_attn_map_2d = normalized_attn_map.unsqueeze(0)  # 添加一个维度，使其成为 [1, height, width]
#         normalized_attn_map_2d = normalized_attn_map_2d.reshape(16,16)
#         # 转换为PIL图像
#         attn_map_image = to_pil_image(normalized_attn_map_2d)
   

#         # 保存注意力图像到本地
#         attn_map_image.save('attn_map_image.png')

#         print("Attention map saved as '/mnt/data/attn_map_image.png'")
    ...
    def attention_masking(
        ...
        attn_mask = attn_map.reshape(B, 64, attn_res ** 2, attn_res ** 2).mean(1, keepdim=False).sum(1, keepdim=False) > 1.0
    ...
    def p_sample(
        ...
        # guide_scale = -0.05
        guide_scale = -0.3
        cond = self.q_sample(model_kwargs["ref_img"], t)
        ...
        attn_eps = mask_out["eps"]
        guided_eps = attn_eps + 2*(guided_eps - attn_eps)
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            model_output,_ = model(x_t, self._scale_timesteps(t), **model_kwargs)
