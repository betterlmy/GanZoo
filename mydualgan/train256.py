...
config_file = "config_mydualgan_256.yaml"
    ...
    for epoch in range(model_config["epoch"], train_config["n_epochs"]):
        ...
        if (
            ...
            torch.save(
                ...
                "mydualgan/saved_models/256/db_generator_%d.pth" % epoch,
            )
            torch.save(
                db_discriminator.state_dict(),
                "mydualgan/saved_models/256/db_discriminator_%d.pth" % epoch,
            )
            torch.save(
                b_generator.state_dict(),
                "mydualgan/saved_models/256/b_generator_%d.pth" % epoch,
            )
            torch.save(
                b_discriminator.state_dict(),
                "mydualgan/saved_models/256/b_discriminator_%d.pth" % epoch,
            )
...
def sample_images(batches_done):
    ...
    save_image(
        train_image_grid,
        "mydualgan/outputs/256/%s.png" % batches_done,
        nrow=5,
        normalize=True,
    )
