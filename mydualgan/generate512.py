...
path_to_images = "/root/lmy/aapm512/high"
...
def geneULDCT():
    ...
    b_generator.load_state_dict(
        torch.load(
            "mydualgan/saved_models/512/b_generator_1200.pth", map_location=device
        )
    )
    b_generator.eval()
    print("model loaded")

    output_folder = "/root/lmy/aapm512/gene_ULDCT"
...
def geneNDCT():
    # sample_images()
    dataset = GeneDataset("/root/lmy/aapm512/gene_ULDCT", transforms_)
    dataloader = DataLoader(
        dataset,
        batch_size=batch_size,
        shuffle=False,
    ...
    db_generator.load_state_dict(
        torch.load(
            "mydualgan/saved_models/512/db_generator_1500.pth", map_location=device
        )
    )
    db_generator.eval()
    print("model loaded")

    output_folder = "/root/lmy/aapm512/gene_NDCT"
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)
    for i, images in enumerate(dataloader):
        print(f"processing batch {i}")
