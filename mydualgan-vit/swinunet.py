...
class WindowAttention(nn.Module):
    ...
    def window_partition(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = rearrange(x, 'B (Nh Mh) (Nw Mw) C -> (B Nh Nw) Mh Mw C',
                      Nh=H // self.window_size, Nw=W // self.window_size)
        return x
...
class SwinUnetDecoder(nn.Module):
    def __init__(self,
        ...
        self.head = nn.Conv2d(
            in_channels=embed_dim, out_channels=num_classes, kernel_size=(1, 1), bias=False)
...
if __name__ == '__main__':
    ...
    model = get_swinunet(img_size=256, in_channels=1)
    y = model(x)
    print(y.shape)
