...
class TrainTask(object):
    ...
    def build_default_options():
        parser = argparse.ArgumentParser(
            "Default arguments for training of different methods"
        )
        ...
        parser.add_argument(
            "--cr_loss_weight",
            type=float,
            default=5.08720932695335,
            help="cr_loss_weight",
        )
        parser.add_argument(
            "--test_batch_size", type=int, default=1, help="test_batch_size"
        )
        parser.add_argument(
            "--num_workers", type=int, default=16, help="num of workers to use"
        )
        parser.add_argument(
            "--max_iter", type=int, default=20000, help="number of training epochs"
        )
        parser.add_argument(
            "--resume_iter", type=int, default=2500, help="number of training epochs"
        )
        ...
        parser.add_argument(
            "--run_name", type=str, default="default", help="each run name"
        )
        parser.add_argument(
            "--model_name", type=str, help="the type of method", default="supcon"
        )
        ...
        parser.add_argument(
            "--warmup_epochs", type=int, default=0, help="warmup epochs"
        )
        return parser
    ...
    def set_loader(self):
        ...
        train_dataset = dataset_dict[opt.train_dataset_name](
            hu_range=(opt.hu_min, opt.hu_max), transforms=None
        )
        ...
        test_dataset = dataset_dict[opt.test_dataset_name](
            hu_range=(opt.hu_min, opt.hu_max)
        )
        test_images = [
            test_dataset[i] for i in range(0, min(300, len(test_dataset)), 50)
        ]
    ...
    def fit(self):
        ...
        for n_iter in tqdm.trange(
            opt.resume_iter + 1, opt.max_iter + 1, disable=(self.rank != 0)
        ):
            inputs = next(loader)
    ...
    def generate_images(self, n_iter):
        ...
        fake_imgs = [low_dose, full_dose, self.generator(low_dose).clamp(0.0, 1.0)]
        fake_imgs = torch.stack(fake_imgs).transpose(1, 0).reshape((-1, ch, w, h))
        self.logger.save_image(
            torchvision.utils.make_grid(fake_imgs, nrow=3), n_iter, "test"
        )
