...
class DUGAN(TrainTask):
    ...
    def set_model(self):
        ...
        g_optimizer = torch.optim.Adam(
            generator.parameters(), opt.g_lr, weight_decay=opt.weight_decay
        )

        self.gan_metric = ls_gan
        img_discriminator = UNet(
            repeat_num=opt.repeat_num,
    ...
    def train_discriminator(
        ...
        disc_loss = (
            self.gan_metric(real_enc, 1.0)
            + self.gan_metric(real_dec, 1.0)
            + self.gan_metric(fake_enc, 0.0)
            + self.gan_metric(fake_dec, 0.0)
            + self.gan_metric(source_enc, 0.0)
            + self.gan_metric(source_dec, 0.0)
        )
        total_loss = disc_loss

        apply_cutmix = self.apply_cutmix_prob[n_iter - 1] < warmup(
            opt.cutmix_warmup_iter, opt.cutmix_prob, n_iter
        )
        if apply_cutmix:
            ...
            cutmix_enc, cutmix_dec = discriminator(
                mask_src_tgt(full_dose, gen_full_dose.detach(), mask)
            )

            cutmix_disc_loss = self.gan_metric(cutmix_enc, 0.0) + self.gan_metric(
                cutmix_dec, mask
            )
    ...
    def update_moving_average(self):
        ...
        for old_param, new_param in zip(
            self.ema_generator.parameters(), self.generator.parameters()
        ):
            old_param.data = old_param.data * m + new_param.data * (1.0 - m)
    ...
    def train(self, inputs, n_iter):
        ...
        if n_iter % opt.d_iter == 0:
            ...
            img_gen_loss = self.gan_metric(img_gen_enc, 1.0) + self.gan_metric(
                img_gen_dec, 1.0
            )
            ...
            if opt.use_grad_discriminator:
                ...
                grad_gen_loss = self.gan_metric(grad_gen_enc, 1.0) + self.gan_metric(
                    grad_gen_dec, 1.0
                )
            ...
            total_loss += (
                img_gen_loss * opt.img_gen_loss_weight
    ...
    @torch.no_grad()
    def generate_images(self, n_iter=0):
        opt = self.opt
        ...
        gen_full_dose = self.generator(low_dose).clamp(0.0, 1.0)
        fake_imgs = [
            low_dose,
            full_dose,
            gen_full_dose,
            self.img_discriminator(gen_full_dose)[1].clamp(0.0, 1.0),
            self.grad_discriminator(self.sobel(gen_full_dose))[1].clamp(0.0, 1.0),
        ]
        fake_imgs = torch.stack(fake_imgs).transpose(1, 0).reshape((-1, ch, w, h))
        self.logger.save_image(
            torchvision.utils.make_grid(fake_imgs, nrow=5), n_iter, "test"
        )
    ...
    def test(self, n_iter):
        ...
        for name, generator in zip(["ema_", ""], [self.ema_generator, self.generator]):
            psnr_score, ssim_score, rmse_score, total_num = 0.0, 0.0, 0.0, 0
            for low_dose, full_dose in tqdm.tqdm(self.test_loader, desc="test"):
                batch_size = low_dose.size(0)
                low_dose, full_dose = low_dose.cuda(), full_dose.cuda()
                gen_full_dose = generator(low_dose).clamp(0.0, 1.0)
